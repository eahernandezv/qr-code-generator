...
describe('App integration', () => {
  ...
  it('builds a real request, renders Creator candidates, then invalidates them on edits', async () => {
    ...
    const sizeToggle = screen.getByRole('group', { name: 'Validated size options' })
    expect(sizeToggle).toHaveTextContent(/Small/)
    expect(sizeToggle).toHaveTextContent(/Medium/)
    expect(sizeToggle).toHaveTextContent(/Large/)
    expect(screen.getByRole('button', { name: /Medium/ })).toHaveAttribute('aria-pressed', 'true')
    await user.click(screen.getByRole('button', { name: /Large/ }))
    expect(screen.getByRole('button', { name: /Large/ })).toHaveAttribute('aria-pressed', 'true')
    expect(screen.getByTestId('selected-image-fit-candidate')).toHaveAttribute('data-candidate-id', `${q9Balanced.candidate_id}-large`)
    ...
    expect(screen.getByRole('button', { name: 'Export denied by Core gates' })).toBeDisabled()
    expect(screen.getByTestId('image-fit-export-blockers')).toHaveTextContent(/preview_export_parity_not_proven/)
  ...
  it('fails closed when Creator generation is unavailable and never shows fixture evidence', async () => {
    ...
    render(<App />)
    await user.click(screen.getByRole('button', { name: 'Generate candidates' }))
    expect(await screen.findByRole('alert')).toHaveTextContent(/Image-Fit did not qualify.*service is unavailable/i)
    expect(screen.queryByTestId('selected-image-fit-candidate')).not.toBeInTheDocument()
    expect(screen.queryByRole('article')).not.toBeInTheDocument()
