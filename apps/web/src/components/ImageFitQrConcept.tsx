...
export default function ImageFitQrConcept() {
  ...
  return <main data-testid="image-fit-qr-concept" data-schema-version="image-fit-qr-api.v1" data-export-payload-bound="false" data-checkout-bound="false" className="min-h-[100dvh] bg-[#070b16] text-white">
    ...
    <div className="mx-auto grid max-w-6xl gap-3 p-3 lg:grid-cols-[minmax(0,.9fr)_minmax(360px,1.1fr)]">
      <section aria-label="How to test" className="lg:col-span-2 rounded-2xl border border-indigo-300/20 bg-indigo-500/10 px-3 py-2 text-[11px] leading-relaxed text-indigo-100"><strong className="text-white">Test in five steps:</strong> 1. choose or keep the target image; 2. choose or keep the destination; 3. press <strong>Generate candidates</strong>; 4. review Readable, Balanced, and Image-first evidence returned by Creator; 5. remember export is not available yet.</section>
      ...
      <section aria-label="Image-Fit QR controls" className="grid content-start gap-2.5 rounded-2xl border border-white/10 bg-slate-900/65 p-3">
        <div className="grid grid-cols-[1fr_auto] items-end gap-2 rounded-xl border border-white/10 bg-slate-950/60 p-2.5"><label className="min-w-0"><span className="mb-1 block text-[10px] font-black uppercase tracking-[.16em] text-slate-400">Target image</span><span className="block truncate text-xs">{targetImage.image_ref}</span></label><label className="cursor-pointer rounded-lg border border-indigo-300/50 bg-indigo-500/20 px-3 py-2 text-[10px] font-bold">Choose image<input aria-label="Choose target image" type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) change(setTargetImage, { ...targetImage, image_ref: file.name, mime_type: file.type as ImageFitRequestV1['target_image']['mime_type'], sha256: '0'.repeat(64) }) }} /></label></div>
