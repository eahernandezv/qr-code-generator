...
async function downloadAuthoritativeArtifact(candidate: ImageFitCandidateV1) {
  const decision = imageFitExportDecision(candidate)
  if (!decision.allowed || !decision.artifact) return
  const bytes = await fetchVerifiedArtifactBytes(decision.artifact)
  const mimeType = decision.artifact.kind === 'export_png' ? 'image/png' : 'image/svg+xml'
...
export default function ImageFitQrConcept() {
  ...
  const setLevel1Payload = useStudioStore((state) => state.setPayload)
  ...
  const preserveLevel1Payload = () => {
    if (destinationValid) setLevel1Payload({ raw: destination, normalized: destination, mode: 'url' })
  }
  ...
  return <main data-testid="image-fit-qr-concept" data-schema-version="image-fit-qr-api.v1" data-export-payload-bound={exportDecision?.allowed ? 'true' : 'false'} data-checkout-bound="false" className="min-h-[100dvh] bg-[#070b16] text-white">
    <header className="border-b border-white/10 bg-slate-950/90 px-4 py-3"><div className="mx-auto flex max-w-6xl items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-indigo-300">Deterministic Studio</p><h1 className="text-base font-bold">Q7 Image-Fit</h1><nav aria-label="QR integration mode" className="mt-2 flex gap-2 text-[10px] font-bold"><a href="/" onClick={preserveLevel1Payload} className="rounded-lg border border-emerald-300/40 px-2 py-1 text-emerald-200">Level 1 Safe</a><span aria-current="page" className="rounded-lg border border-indigo-300/50 bg-indigo-500/20 px-2 py-1 text-indigo-100">Q7 Image-Fit Premium</span></nav></div><span className="rounded-full border border-amber-300/30 bg-amber-300/10 px-2.5 py-1 text-[10px] font-bold text-amber-200">Core authority required</span></div></header>
    <div className="mx-auto grid max-w-6xl gap-3 p-3 lg:grid-cols-[minmax(0,.9fr)_minmax(360px,1.1fr)]">
      ...
      <section aria-label="Image-Fit QR preview" className="rounded-2xl border border-white/10 bg-slate-900/60 p-3">
        ...
        {selected && <><button type="button" disabled={!exportDecision?.allowed} onClick={() => { void downloadAuthoritativeArtifact(selected).catch((caught) => setError(caught instanceof Error ? caught.message : 'Core-authorized download failed closed.')) }} className="mt-2 min-h-11 w-full rounded-xl bg-indigo-500 px-4 text-xs font-black text-white disabled:cursor-not-allowed disabled:bg-slate-800 disabled:text-slate-400">{exportDecision?.allowed ? 'Download Core-authorized artifact' : 'Export denied by Core gates'}</button>{!exportDecision?.allowed && <p data-testid="image-fit-export-blockers" className="mt-1 text-[10px] text-amber-200">Blocked: {exportDecision?.blockers.join(', ') || 'Core export authority unavailable'}</p>}</>}
        {error && <div role="alert" className="mt-3 rounded-xl border border-rose-300/30 bg-rose-400/10 px-3 py-2 text-xs text-rose-100"><strong>Image-Fit did not qualify.</strong> {error} Previous candidates remain hidden. <a href="/" onClick={preserveLevel1Payload} className="font-bold underline">Continue with deterministic Level 1 Safe</a>.</div>}
        <div role="status" className="mt-3 rounded-xl border border-amber-300/25 bg-amber-300/10 px-3 py-2 text-[10px] text-amber-100">Only Core-authorized exact bytes can download. Payment, committed short-link, scan, parity, and Image-first experimental blockers remain visible and fail closed. <a href="/" onClick={preserveLevel1Payload} className="font-bold underline">Level 1 Safe remains available</a>.</div>
      </section>
