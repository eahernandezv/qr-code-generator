...
const evidenceDir = path.resolve(process.cwd(), '../../docs/program/evidence/level2-real-generation-ui')
...
test('mobile real request binding, candidate rendering, invalidation, and fail-closed outage', async ({ page }) => {
  ...
  await page.route('**/api/artistic-qr/image-fit/candidates', async (route) => {
    ...
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ ...fixture, request: { ...fixture.request, request_id: request.request_id } }) })
  })
  ...
  await expect(page.getByText(/Test in five steps/)).toBeVisible()
  ...
  await expect(page.getByRole('article', { name: 'Balanced generated candidate' })).toContainText('8/8 checks')
