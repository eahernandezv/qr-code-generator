...
const output = resolve(root, 'docs/program/evidence/q9-quality-loop/cycle-5-background-trim');
...
const candidates = result.response.candidates.map((candidate) => {
  ...
  return {
    ...
    product_visual_score: { ...scored, delta_from_q9_cycle2: scored.total === null ? null : scored.total - q8Scores[candidate.mode] },
  };
});
...
const objective = {
  schema_version: 'q9-cycle-5-objective.v1', baseline_commit: '234d759d978de159f105dcdfce0aeb3b1e275f9c',
...
const scores = {
  schema_version: 'q9-product-visual-quality-scores.v1', cycle: 5, scorer: 'QR Product Architect direct', scoring_note: 'Independent Product visual score, not Creator telemetry.',
...
console.log(JSON.stringify({ output, candidates: candidates.map((candidate)=>({ mode:candidate.mode, total:candidate.product_visual_score.total, delta:candidate.product_visual_score.delta_from_q9_cycle2, checks:`${candidate.independent_validation.checks_passed}/8`, hash:candidate.artifact.sha256 })) }, null, 2));
