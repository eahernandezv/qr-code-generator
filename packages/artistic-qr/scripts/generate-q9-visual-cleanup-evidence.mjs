...
const baselineRoot = '/home/hermes/qr-mvp-demo';
const output = resolve(root, 'docs/program/evidence/q9-visual-cleanup');
...
function savePng(validation, candidate, artifact, path) {
  writeFileSync(path.replace(/\.png$/, '.svg'), artifact.data);
  const raster = validation.rasterizeCandidate(validationCandidate(candidate, artifact));
  const png = new PNG({ width: raster.width, height: raster.height });
  png.data = Buffer.from(raster.data);
  writeFileSync(path, PNG.sync.write(png));
}
async function runVariant(id, variantRoot) {
  const imageFit = await import(pathToFileURL(resolve(variantRoot, 'packages/artistic-qr/dist/image-fit.js')).href);
  const validation = await import(pathToFileURL(resolve(variantRoot, 'packages/artistic-qr/dist/validation.js')).href);
  const result = imageFit.optimizeImageFitQr(input, { _visualPolicy: 'q9_negative_space_showcase' });
  const dir = resolve(output, id);
  mkdirSync(dir, { recursive: true });
  const records = [];
  for (const candidate of result.response.candidates) {
    const artifact = result.artifacts[candidate.candidate_id];
    const independent = validation.runValidation(validationCandidate(candidate, artifact), payload);
    const label = `${candidate.image_treatment.logo_size}-${Math.round((candidate.image_treatment.logo_size_fraction ?? 0) * 100)}`;
    savePng(validation, candidate, artifact, resolve(dir, `${label}.png`));
    records.push({
      label, size: candidate.image_treatment.logo_size, fraction: candidate.image_treatment.logo_size_fraction,
      candidate_id: candidate.candidate_id, artifact_sha256: artifact.sha256,
