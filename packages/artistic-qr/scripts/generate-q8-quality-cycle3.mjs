...
const output = resolve(root, 'docs/program/evidence/q8-quality-loop/cycle-3-generalization');
...
for(const definition of targets){
  ...
  const options={_visualPolicy:'q8_negative_space_island'},result=optimizeImageFitQr(input,options),repeat=optimizeImageFitQr(input,options),artifactDir=resolve(output,definition.id,'artifacts');mkdirSync(artifactDir,{recursive:true});
...
writeFileSync(resolve(output,'objective-evidence.json'),JSON.stringify({schema_version:'q8-cycle-3-generalization.v1',visual_policy:'q8_negative_space_island',payload_sha256:createHash('sha256').update(payload).digest('hex'),targets:evidenceTargets},null,2)+'\n');
